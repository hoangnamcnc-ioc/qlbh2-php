...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($shipment) {
        $pdo->prepare('UPDATE shipments SET tracking_code=?, carrier_name=?, status=?, shipping_fee=?, cod_amount=?, note=? WHERE id=?')
            ->execute([$trackingCode, $carrierName, $status, $shippingFee, $codAmount, $note, $shipment['id']]);
    } else {
        $pdo->prepare('INSERT INTO shipments (order_id, tracking_code, carrier_name, status, shipping_fee, cod_amount, note, created_by_id) VALUES (?,?,?,?,?,?,?,?)')
            ->execute([$orderId, $trackingCode, $carrierName, $status, $shippingFee, $codAmount, $note, $currentUser['id']]);
    }
