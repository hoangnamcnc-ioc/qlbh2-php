...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'assign_price_list') {
        ...
        if ($priceListId) {
            $plCheck = $pdo->prepare('SELECT id FROM price_lists WHERE id = ? AND tenant_id = ?');
            $plCheck->execute([$priceListId, $tenantId]);
            if (!$plCheck->fetch()) {
                $priceListId = null;
            }
        }
        $pdo->prepare('UPDATE customer_groups SET price_list_id = ? WHERE id = ? AND tenant_id = ?')->execute([$priceListId, $groupId, $tenantId]);
        redirect('groups.php');
    } else {
        $name = post('name');
        ...
        if ($name === '') {
            ...
            $check = $pdo->prepare('SELECT id FROM customer_groups WHERE name = ? AND tenant_id = ?');
            ...
            if ($check->fetch()) {
                ...
                $codeCheck = $pdo->prepare('SELECT id FROM customer_groups WHERE code = ? AND tenant_id = ?');
                $codeCheck->execute([$code, $tenantId]);
                if ($codeCheck->fetch()) {
                    $error = 'Mã nhóm này đã tồn tại';
                } else {
                    $pdo->prepare('INSERT INTO customer_groups (name, code, description, tenant_id) VALUES (?,?,?,?)')->execute([$name, $code, $description, $tenantId]);
                    redirect('groups.php');
                }
            }
        }
...
$groupsStmt = $pdo->prepare(
    'SELECT g.*, (SELECT COUNT(*) FROM customers c WHERE c.group_id = g.id AND c.tenant_id = ?) AS customer_count, pl.name AS price_list_name
     FROM customer_groups g LEFT JOIN price_lists pl ON pl.id = g.price_list_id
     WHERE g.tenant_id = ?
     ORDER BY g.name'
);
$groupsStmt->execute([$tenantId, $tenantId]);
